...
export function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      const [dashRes, groupsRes, workloadRes] = await Promise.all([
      ...
      // Use supervisor workload count (matches Supervisor Assignment page)
      if (workloadRes.success && workloadRes.data?.totalStats?.totalSupervisors != null) {
        ...
        setSupervisorCount(dashRes.data?.totals?.supervisors ?? null);
      }
      setLoading(false);
    };
    fetchDashboard();
